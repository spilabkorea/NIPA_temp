...
const compSchema_reg = new mongoose.Schema({
    _id : {type : Object},
    compressorId : {type : String},
    time : {type : Number},
    compressorpressure : {type : Number},
    compressortemperature : {type : Number},
    compressorexternaltemperature : {type : Number},
    compressorexternalhumidity : {type : Number},
    grmsx : {type : Number},
    grmsy : {type : Number},
    grmsz : {type : Number},
    compressorstatus : {type : Number}
  }
  );

// compressorId 정보 담기
const compIdList = ['364']
...
compIdList.forEach((id)=>{
    let model = mongoose.model(`compressor${id}_regression`, compSchema_reg, `compressor${id}_regression`);
    compressor_lst[`compressor${id}_regression`] = model;
  })
...
function getCompressorModel_reg(compId){
    return compressor_lst[`compressor${compId}_regression`]
}
...
exports.getCompressorByTime_reg = (req,res) =>{
    const {compId} = req.params;
    const startTime = req.query.startTime;
    const endTime = req.query.endTime;

    // 파라미터에 맞는 데이터 조회
    var Compressor = getCompressorModel_reg(compId)
    Compressor
        .find()
        .where('time')
        .gte(startTime)
